...
(defcustom doxymacs-external-xml-parser-executable
  "/e/home/bin/doxymacs_parser.exe"
...
(defun doxymacs-current-time-string()
  (setq curr-date (calendar-current-date))
  (setq weekth (cal-china-x-week-of-date curr-date))
  (setq week (car (split-string (current-time-string))))
  (setq day (format-time-string "%Y-%m-%d %T"))
  (format "W%s %s, %s"
          weekth
          week
          day))
...
(defconst doxymacs-JavaDoc-file-comment-template
 '("/**" > n
   ...
   (if (buffer-file-name)
     ...
     " * " (doxymacs-doxygen-command-char) "author  " "zhangxc, zhangxc@leedarson.com" > n
     " * " (doxymacs-doxygen-command-char) "version v1.0.0" > n
   " * " (doxymacs-doxygen-command-char) "date    " (doxymacs-current-time-string) > n
   " * " (doxymacs-doxygen-command-char) "par     Copyright" > n
   " * " "Copyright (c) Leedarson Lighting 2000-2015. All rights reserved." > n
   " *" > n
   " * " (doxymacs-doxygen-command-char) "par     History" > n
   " * " "1.Date        :" (doxymacs-current-time-string) > n
   " * " "  Author      :" "zhangxc" > n
   " * " "  Version     :" "v1.0.0" > n
   " * " "  Modification:" "Create file" > n
   " */" > n)

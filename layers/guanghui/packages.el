...
(defun guanghui/init-blog-admin ()
  (use-package blog-admin
    :init
    (progn
      ;; do your configuration here
      (setq blog-admin-backend-path "~/note/blog")
...
(defun guanghui/post-init-projectile ()
  ...
  current project. Otherwise do `occur' in the current file."
    ...
    (spacemacs/set-leader-keys "ps" 'my-simple-todo)))

(defun guanghui/post-init-org ()
  (with-eval-after-load 'org
    (progn
      ;; define the refile targets
      (setq org-agenda-files (quote ("~/note/person/" )))
      (setq org-default-notes-file "~/note/person/life.org")
      ...
      (setq org-capture-templates
            '(("t" "Todo" entry (file+headline "~/note/person/gtd.org" "代表事项")
               "* TODO [#B] %?\n  %i\n"
               :empty-lines 1)
              ("m" "Morning Diary" entry (file+headline "~/note/person/life.org" "清晨日志")
               "* %?\n %i\n"
               :empty-lines 1)
              ("n" "notes" entry (file+headline "~/note/person/notes.org" "Quick notes")
               "* %?\n  %i\n %U"
               :empty-lines 1)
              ("b" "Blog Ideas" entry (file+headline "~/note/person/notes.org" "Blog Ideas")
               "* TODO [#B] %?\n  %i\n %U"
               :empty-lines 1)
              ("s" "Code Snippet" entry
               (file "~/note/person/snippets.org")
               "* %?\t%^g\n#+BEGIN_SRC %^{language}\n\n#+END_SRC")
              ("p" "Project" entry (file+headline "~/note/person/project.org" "项目安排")
               "* TODO [#A] %?\n  %i\n %U"
               :empty-lines 1)
              ("c" "Chrome" entry (file+headline "~/note/person/notes.org" "Quick notes")
               "* TODO [#C] %?\n %(zilongshanren/retrieve-chrome-current-tab-url)\n %i\n %U"
               :empty-lines 1)
              ("l" "links" entry (file+headline "~/note/person/bookmark.org" "网址收藏")
               "* TODO [#C] %?\n  %i\n %a \n %U"
               :empty-lines 1)
              ("j" "Journal Entry"
               entry (file+datetree "~/note/person/journal.org")
               "* %?"
               :empty-lines 1)))
